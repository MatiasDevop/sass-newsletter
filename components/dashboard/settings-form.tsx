...
export function SettingsForm({ initialSettings }: SettingsFormProps) {
  ...
  const [formData, setFormData] = React.useState<UserSettingsInput>({
    ...
    companyName: initialSettings?.companyNaeme || "",
  ...
  const handleSave = () => {
    ...
    upsertUserSettings(cleanedData).then(
      () => {
  ...
  const handleChange: FieldChangeHandler = (field, value) => {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <BasicInformationCard formData={formData} onChange={handleChange} />
      <BrandIdentityCard formData={formData} onChange={handleChange} />
      ...
      />
      <SenderInformationCard formData={formData} onChange={handleChange} />
      <SaveButtonBar onSave={handleSave} isSaving={isSaving} />
    </div>
  );
}
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => onChange("newsletterName", e.target.value)}
          />
        </div>
...
function SenderInformationCard({
  formData,
  onChange,
}: {
  formData: UserSettingsInput;
